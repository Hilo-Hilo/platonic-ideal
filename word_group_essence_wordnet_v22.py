#!/usr/bin/env python3
"""
V2.2: Build on v2.1 with advanced quality improvements:

Includes all v2.1 features:
- Word frequency filtering (WordNet ∩ wordfreq)
- Tokenization-invariant embedding

Plus three advanced improvements:
1) Robust group centers (trimmed mean)
   - Drop outlier entries by cosine-to-preliminary-mean, recompute mean
   - Reduces impact of noisy/polysemous entries

2) Sense-aware reranking (WordNet gloss mix)
   - Rerank top candidates using synset (sense) vectors
   - u_synset = alpha * v_lemma + (1-alpha) * v_gloss
   - Targets polysemy failures

3) Diagonal whitening
   - After centering/ABT, scale each dimension by inverse std dev
   - Lightweight O(N·D) whitening for better isotropy
...
# V2.2 defaults
DEFAULT_ROBUST_GROUP_CENTER = True
DEFAULT_TRIM_FRACTION = 0.2
DEFAULT_SENSE_RERANK_ENABLED = True
DEFAULT_SENSE_ALPHA = 0.7
DEFAULT_SENSE_RERANK_POOL_MULTIPLIER = 10
DEFAULT_DIAG_WHITEN_ENABLED = True
DEFAULT_DIAG_WHITEN_EPS = 1e-6
...
def _trimmed_mean_vectors(vecs: List[np.ndarray], trim_fraction: float) -> np.ndarray:
    """
    Compute trimmed mean: drop bottom trim_fraction by cosine-to-preliminary-mean.
    Returns normalized trimmed mean vector.
    """
    if len(vecs) <= 2 or trim_fraction <= 0:
        # Not enough vectors or no trimming requested
        vecs_array = np.stack(vecs, axis=0)
        mean_vec = vecs_array.mean(axis=0, dtype=np.float32)
        return _normalize_vector(mean_vec)
    
    # Preliminary mean
    vecs_array = np.stack(vecs, axis=0)
    prelim_mean = vecs_array.mean(axis=0, dtype=np.float32)
    prelim_mean_norm = _normalize_vector(prelim_mean)
    
    # Compute cosine similarity of each vector to preliminary mean
    cosines = []
    for v in vecs:
        v_norm = _normalize_vector(v)
        cos_sim = float(np.dot(v_norm, prelim_mean_norm))
        cosines.append(cos_sim)
    
    # Sort by cosine (ascending), drop bottom trim_fraction
    n_drop = max(1, int(len(vecs) * trim_fraction))
    sorted_indices = np.argsort(cosines)
    keep_indices = sorted_indices[n_drop:]
    
    # Recompute mean from kept vectors
    kept_vecs = [vecs[i] for i in keep_indices]
    kept_array = np.stack(kept_vecs, axis=0)
    trimmed_mean = kept_array.mean(axis=0, dtype=np.float32)
    return _normalize_vector(trimmed_mean)


def _diagonal_whiten_transform(
    candidate_vecs: np.ndarray,
    eps: float = 1e-6
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Diagonal whitening: center, then scale each dimension by inverse std dev.
    
    Returns:
        mu: (D,) mean vector
        inv_std: (D,) inverse std dev per dimension
    """
    mu = candidate_vecs.mean(axis=0, dtype=np.float32)
    X_centered = candidate_vecs - mu
    
    std = X_centered.std(axis=0, dtype=np.float32)
    std = np.where(std > eps, std, 1.0)  # Avoid division by zero
    inv_std = 1.0 / std
    
    return mu, inv_std


def _sense_aware_rerank(
    wn,
    top_candidates: List[Tuple[float, str, List[int]]],
    embeddings: np.ndarray,
    tokenizer,
    ...
    *,
    ...
    """
    Rerank candidates using synset (sense) vectors.
    For each candidate word, find its synsets, compute sense vectors as:
        u_synset = alpha * v_lemma + (1-alpha) * v_gloss
    Score each synset, pick best synset score for the word.
    """
    reranked = []
    
    for orig_score, word, token_ids in top_candidates:
        # Get synsets for this word
        synsets = wn.synsets(word.lower())
        
        if not synsets:
            # No synsets found, keep original score
            reranked.append((orig_score, word, token_ids))
            continue
        
        # Get lemma vector (already computed, but we'll recompute for consistency)
        v_lemma, _, _ = _text_vector(
            embeddings, tokenizer, word,
            use_spherical_mean=True,
            tokenization_invariance=tokenization_invariance
        )
        
        best_synset_score = orig_score
        ...
        for synset in synsets:
            ...
            # Compute gloss vector
            try:
                v_gloss, _, _ = _text_vector(
                    embeddings, tokenizer, gloss,
                    use_spherical_mean=True,
                    tokenization_invariance=tokenization_invariance
                )
            except Exception:
            ...
            # Mix lemma + gloss
            u_synset = sense_alpha * v_lemma + (1 - sense_alpha) * v_gloss
            u_synset = _normalize_vector(u_synset)
            
            # Score this synset
            if use_per_group_scoring:
                score = 0.0
                for gm, gw in zip(group_mean_vectors, group_weights):
                    gm_norm = _normalize_vector(gm)
                    cos_sim = float(np.dot(u_synset, gm_norm))
                    score += gw * cos_sim
            else:
                # Single essence vector
                weighted_sum = np.zeros_like(group_mean_vectors[0])
                for gm, gw in zip(group_mean_vectors, group_weights):
                    weighted_sum += gw * gm
                essence_vec = weighted_sum / len(group_mean_vectors)
                essence_vec = _normalize_vector(essence_vec)
                score = float(np.dot(u_synset, essence_vec))
            
            if score > best_synset_score:
                best_synset_score = score
        
        reranked.append((best_synset_score, word, token_ids))
...
def _compute_essence_vector_v22(
    doc: Dict[str, Any],
    embeddings: np.ndarray,
    tokenizer,
    *,
    ...
    use_spherical_mean: bool,
    tokenization_invariance: bool,
    robust_group_center: bool,
    trim_fraction: float,
) -> Tuple[List[Dict[str, Any]], List[np.ndarray], List[float], List[str], bool]:
    """
    V2.2: Same as v2.1, but optionally uses trimmed mean for group centers.
    """
    ...
    for gi, group in enumerate(groups_in):
        ...
        # Compute group mean (with optional trimmed mean for robustness)
        if robust_group_center and len(entry_vecs) > 2:
            # Use trimmed mean (drops outliers)
            group_mean = _trimmed_mean_vectors(entry_vecs, trim_fraction)
        elif use_spherical_mean:
...
def _rank_top_wordnet_words_v22(
    embeddings: np.ndarray,
    tokenizer,
    group_mean_vectors: List[np.ndarray],
    group_weights: List[float],
    *,
    ...
    freq_top_n: int,
    freq_min_zipf: Optional[float],
    diag_whiten_enabled: bool,
    diag_whiten_eps: float,
    sense_rerank_enabled: bool,
    sense_alpha: float,
    sense_rerank_pool_multiplier: int,
    ...
    # Apply diagonal whitening if enabled (after ABT)
    if diag_whiten_enabled:
        whiten_mu, inv_std = _diagonal_whiten_transform(X, diag_whiten_eps)
        X = (X - whiten_mu) * inv_std
    else:
        whiten_mu = None
        inv_std = None
    ...
    for gm in group_mean_vectors_normalized:
        ...
        if diag_whiten_enabled and whiten_mu is not None:
            gm_c = (gm_c - whiten_mu) * inv_std
    ...
    # Apply sense-aware reranking if enabled
    if sense_rerank_enabled:
        # Rerank a larger pool (top_k * multiplier)
        rerank_pool_size = min(len(best), top_k * sense_rerank_pool_multiplier)
        rerank_pool = best[:rerank_pool_size]
        reranked = _sense_aware_rerank(
            wn,
            rerank_pool,
            embeddings,
            tokenizer,
    ...
    stats: Dict[str, Any] = {
        ...
        "v22_features": {
            ...
            "diag_whiten_enabled": bool(diag_whiten_enabled),
            "sense_rerank_enabled": bool(sense_rerank_enabled),
            "sense_alpha": float(sense_alpha) if sense_rerank_enabled else 0.0,
...
def compute_essence_wordnet_v22(
    doc: Dict[str, Any],
    *,
    ...
    robust_group_center: bool = DEFAULT_ROBUST_GROUP_CENTER,
    trim_fraction: float = DEFAULT_TRIM_FRACTION,
    diag_whiten_enabled: bool = DEFAULT_DIAG_WHITEN_ENABLED,
    diag_whiten_eps: float = DEFAULT_DIAG_WHITEN_EPS,
    sense_rerank_enabled: bool = DEFAULT_SENSE_RERANK_ENABLED,
    sense_alpha: float = DEFAULT_SENSE_ALPHA,
    sense_rerank_pool_multiplier: int = DEFAULT_SENSE_RERANK_POOL_MULTIPLIER,
) -> Dict[str, Any]:
    _validate_input(doc)
    ...
    group_results, group_mean_vectors, group_weights, warnings, any_weight_clipped = (
        _compute_essence_vector_v22(
            ...
            use_spherical_mean=use_spherical_mean,
            tokenization_invariance=tokenization_invariance,
            robust_group_center=robust_group_center,
    ...
    top_words, wordnet_stats = _rank_top_wordnet_words_v22(
        ...
        freq_min_zipf=freq_min_zipf,
        diag_whiten_enabled=diag_whiten_enabled,
        diag_whiten_eps=diag_whiten_eps,
        sense_rerank_enabled=sense_rerank_enabled,
        sense_alpha=sense_alpha,
        sense_rerank_pool_multiplier=sense_rerank_pool_multiplier,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="V2.2: v2.1 + (robust group centers) + (sense-aware reranking) + (diagonal whitening)."
    )
    ...
    # V2.2 options
    parser.add_argument("--no-robust-center", action="store_true", help="Disable trimmed-mean group centers")
    parser.add_argument("--trim-fraction", type=float, default=DEFAULT_TRIM_FRACTION, help="Fraction to trim (default: 0.2)")
    parser.add_argument("--no-diag-whiten", action="store_true", help="Disable diagonal whitening")
    parser.add_argument("--diag-whiten-eps", type=float, default=DEFAULT_DIAG_WHITEN_EPS, help="Whitening epsilon (default: 1e-6)")
    parser.add_argument("--no-sense-rerank", action="store_true", help="Disable sense-aware reranking")
    parser.add_argument("--sense-alpha", type=float, default=DEFAULT_SENSE_ALPHA, help="Sense mix alpha (default: 0.7)")
    parser.add_argument("--sense-rerank-pool", type=int, default=DEFAULT_SENSE_RERANK_POOL_MULTIPLIER, help="Rerank pool multiplier (default: 10)")
    ...
    try:
        out = compute_essence_wordnet_v22(
            doc=doc,
            ...
            robust_group_center=not bool(args.no_robust_center),
            trim_fraction=float(args.trim_fraction),
            diag_whiten_enabled=not bool(args.no_diag_whiten),
            diag_whiten_eps=float(args.diag_whiten_eps),
            sense_rerank_enabled=not bool(args.no_sense_rerank),
            sense_alpha=float(args.sense_alpha),
            sense_rerank_pool_multiplier=int(args.sense_rerank_pool),
        )
    except Exception as e:
