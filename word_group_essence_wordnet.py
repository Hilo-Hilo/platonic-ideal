#!/usr/bin/env python3
"""
Compute a weighted "essence" vector for word groups and then return the nearest
FULL DICTIONARY WORDS using WordNet (nouns/verbs by default).

Why this exists:
- Model vocab tokens are subwords; nearest-token results are often fragments.
- Wordfreq word lists can include abbreviations and corpus artifacts.
- WordNet is a curated lexical database with part-of-speech information.

Input JSON (array-based groups):
{
  "repo_id": "Qwen/Qwen2.5-0.5B",
  "groups": [
    { "name": "space", "weight": 1.0, "entries": ["planet", "earth", "sun"] }
  ]
}

Essence math:
  entry_vec = mean(token_vecs(entry))                 # per entry
  group_mean = mean(entry_vecs in group)              # per group
  group_weighted = weight * group_mean                # apply group weight
  overall = mean(group_weighted over groups)          # equal weight per group

Nearest-neighbor step (word-level):
  - Enumerate WordNet lemma names for selected POS (default: nouns+verbs)
  - Candidate word vector = mean(token_vecs(word))
  - Rank candidate WORDS by cosine similarity to `overall`
  - Return top_k words (default: 20), excluding input words

Output JSON:
  Preserves input + adds `_output` containing norms, top_words, and settings.

Note: This script does NOT write raw embedding vectors to JSON.
"""
...
def _compute_essence_vector(
    doc: Dict[str, Any],
    embeddings: np.ndarray,
    tokenizer,
    *,
    weight_clip_abs: Optional[float],
) -> Tuple[List[Dict[str, Any]], np.ndarray, List[str], bool]:
    groups_in: List[Dict[str, Any]] = doc["groups"]

    group_results: List[Dict[str, Any]] = []
    weighted_group_vectors: List[np.ndarray] = []
    warnings: List[str] = []
    ...
    for gi, group in enumerate(groups_in):
        ...
        for ei, entry in enumerate(entries):
            ...
            entry_vecs.append(_mean_token_vectors(embeddings, token_ids))
            entry_infos.append(
        ...
        weighted_group_vectors.append(group_weighted)
    ...
    overall = (np.stack(weighted_group_vectors, axis=0).mean(axis=0, dtype=np.float32)).astype(
        np.float32, copy=False
    )
    overall_norm = float(np.linalg.norm(overall))
    if not math.isfinite(overall_norm) or overall_norm <= 0:
        raise ValueError("Overall essence vector has zero/invalid norm; check weights and entries")

    return group_results, overall, warnings, any_weight_clipped
...
def _rank_top_wordnet_words(
    ...
    *,
    ...
    if not math.isfinite(q_norm) or q_norm <= 0:
        raise ValueError("Essence vector has zero/invalid norm; cosine similarity undefined")
    ...
    heap: List[Tuple[float, str, List[int]]] = []  # (score, word, token_ids)
    ...
    for tok_len in sorted(buckets.keys()):
        ...
        for start in range(0, len(items), candidate_batch):
            ...
            vecs = embeddings[token_id_mat].mean(axis=1, dtype=np.float32)  # (B, D)
            dot = vecs @ q
            row_sq = np.einsum("ij,ij->i", vecs, vecs, optimize=True)
            row_norm = np.sqrt(row_sq, dtype=np.float32)
            denom = row_norm * np.float32(q_norm)
            denom = np.where(denom > 0, denom, np.float32("inf"))
            sim = (dot / denom).astype(np.float32, copy=False)
            sim = np.where(np.isfinite(sim), sim, np.float32("-inf"))

            local_k = min(top_k * 8, B)
            idx = np.argpartition(sim, -local_k)[-local_k:]
            for j in idx:
                score = float(sim[int(j)])
                w = words[int(j)]
                ids = ids_list[int(j)]
                if len(heap) < top_k:
                    heapq.heappush(heap, (score, w, ids))
                else:
                    if score > heap[0][0]:
                        heapq.heapreplace(heap, (score, w, ids))
...
def compute_essence_wordnet(
    ...
    *,
    ...
    group_results, overall_vec, warnings, any_weight_clipped = _compute_essence_vector(
        doc, embeddings, tokenizer, weight_clip_abs=weight_clip_abs
    )
    ...
    top_words, wordnet_stats = _rank_top_wordnet_words(
        embeddings=embeddings,
        tokenizer=tokenizer,
        query_vec=overall_vec,
    ...
    out_doc["_output"] = {
        ...
        "settings": {
            "entry_vector_rule": "mean_tokens",
            "group_aggregation": "mean_entries",
            "overall_aggregation": "mean_weighted_groups",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute weighted word-group essence vectors and return nearest WordNet dictionary words."
    )
    ...
    parser.add_argument("--min-word-chars", type=int, default=DEFAULT_MIN_WORD_CHARS, help="Minimum characters for candidate words")
    parser.add_argument("--max-token-len", type=int, default=DEFAULT_MAX_TOKEN_LEN, help="Skip candidate words that tokenize to more than this many tokens")
    parser.add_argument("--candidate-batch", type=int, default=DEFAULT_CANDIDATE_BATCH, help="Batch size for scoring candidates (vectorized)")
    parser.add_argument("--no-exclude-input", action="store_true", help="Do not exclude input words from top_words")
    parser.add_argument("--no-exclude-component-words", action="store_true", help="Do not exclude component words from multi-word phrases")
    parser.add_argument(
        "--no-exclude-substrings",
        action="store_true",
        help="Do not exclude candidates that contain input words as substrings (e.g., 'earthworm' for input 'earth')",
    )
    parser.add_argument("--weight-clip-abs", type=float, default=DEFAULT_WEIGHT_CLIP, help="Clip |group weight| to this value (set <=0 to disable)")
    parser.add_argument("--no-weight-clip", action="store_true", help="Disable group weight clipping")
